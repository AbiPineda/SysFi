<?php 
   ...
   if(!empty($_GET['del'])){
        $id=$_GET['del'];
        mysql_query("DELETE FROM venta WHERE articulo='$id'");
                    ...
                    if(!empty($_POST['buscar_cliente'])){
                        ...
                        if($roow=mysql_fetch_array($poa)){
                            ...
                            if($row=mysql_fetch_array($pa)){
                                ...
                                mysql_query("INSERT INTO venta (cliente) VALUES ('$codigoo')");   
                            }
                        }else{
...
<!--######################################## ARTICULOS ############################################################################################## -->
                    ...
                    if(!empty($_POST['new_cant'])){
                        ...
                        mysql_query("UPDATE venta SET cant='$cantf' WHERE articulo='$ncodigo' and usu='$usu'");
                    }
                    if(!empty($_POST['new_pv'])){
                        ...
                        mysql_query("UPDATE venta SET p_mayor='$newp' WHERE articulo='$pvcodigo' and usu='$usu'");
                    }
                    ...
                    if(!empty($_POST['ncodigo_ref'])){
                        ...
                        if($referencia==''){
                            mysql_query("UPDATE venta SET ref='' WHERE articulo='$ncodigo' and usu='$usu' and ref='$ref_ant'");
                        }else{
                            $pa=mysql_query("SELECT * FROM venta WHERE venta.ref='$referencia'");             
                            if($row=mysql_fetch_array($pa)){
                                echo mensajes('El Numero de Referencia "'.$referencia.'" Esta siendo usada','rojo');
                            }else{
                                mysql_query("UPDATE venta SET ref='$referencia' WHERE articulo='$ncodigo' and usu='$usu' and ref='$ref_ant'");
                            }
                        }
                        ...
                        if ($row=mysql_fetch_array($i)) {
                            if ($row['stock'] == 0) {
                                ...
                                echo mensajes('El Articulo '.$oArticulo->consultar('nombre').' se encuentra en Existencia "0"','naranja');
                                  ...
                                  $pa=mysql_query("SELECT * FROM venta WHERE articulo='$codigo' and usu='$usu' and ref=''");   
                                    if($row=mysql_fetch_array($pa)){
                                        $cant=$row['cant']+1;
                                        mysql_query("UPDATE venta SET cant='$cant' WHERE articulo='$codigo' and usu='$usu'");
                                    }else{
                                        mysql_query("INSERT INTO venta (articulo, cliente) VALUES ('$codigo','$usu')");    
                                    }
                ...
                <div class="table-responsive">                                
                        <table class="table table-striped">
                            ...
                            <?php 
                                ...
                                $pa=mysql_query("SELECT * FROM venta, inventario WHERE venta.usu='$usu' and venta.articulo=inventario.articulo");              
                                while($row=mysql_fetch_array($pa)){
                                    $item=$item+$row['cant'];                                   
