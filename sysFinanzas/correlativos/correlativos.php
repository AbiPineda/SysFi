...
class correlativos {
    ...
    public static function lista_departamento_filtrada($conexion, $id) {
        $lista = array();

        if (isset($conexion)) {
            try {
                $sql = "select * from departamento where id_institucion = '$id'";
                $sentencia = $conexion->prepare($sql);
                $sentencia->execute();
                $resultado = $sentencia->fetchAll();
            ...
            try {
                ...
                if (count($resultado)) {
                    foreach ($resultado as $fila) {
                        ...
                        $institucion->setIddepartamento($fila['idInstitucion']);
